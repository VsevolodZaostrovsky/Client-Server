...
int main() {
    DataBase A;
    generateRandomBase("large.txt", 10);
    Result Re;
    A.addFromFile("large.txt");
    std::cout << A.execute("ADDCOMP NAME=NEW_ITEM COMP=D_1 QUANTITY=1 COMP=D_2 QUANTITY=2 COMP=D_3 QUANTITY=3 END", 1).toString(100).c_str() << std::endl;
    std::cout << A.execute("ISPOSSIBLE NAME=NEW_ITEM QUANTITY=1 END", 1).toString(100).c_str() << std::endl;
    std::cout << A.execute("DELITEM NAME=NEW_ITEM END", 1).toString(100).c_str() << std::endl;
    std::cout << A.execute("DELITEM NAME=Nqw_EM END", 1).toString(100).c_str() << std::endl;
